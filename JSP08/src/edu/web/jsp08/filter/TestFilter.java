...
public class TestFilter implements Filter {
	...
	public void destroy() {
    	...
    	System.out.println("testFilter.destroy() 메소드 호출");
	}
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		System.out.println(">>> tesFilter.doFilter() 메서드 호출 전");
		// 요청(request)을 필터 체인을 따라서 전달. -> 요청을 처리ㅏㅎ는 servlet 메서드가 호출
		chain.doFilter(request, response);
		
		
		
		// servlet 함수의 처리가 다 끝났을 때 할 일들을 작성
		System.out.println(">>> tesFilter.doFilter() 메서드 호출 후");
	}
