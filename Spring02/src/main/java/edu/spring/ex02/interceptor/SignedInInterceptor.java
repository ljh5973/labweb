...
// signin POST 방식 controller method가 return 후
// 성공이면 가고자 했었던 url로 이동
// 실패하면 다시 signin GET 요청으로 redirect
public class SignedInInterceptor implements HandlerInterceptor{
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if(request.getMethod().equals("GET")) {
			...
			Object obj=modelAndView.getModel().get("signInUser");
			
			if(obj!=null) { // 테이블에 아이디/패스워드 일치하는 사용자가 있는 경우 -> 로그인 성공
				// session에 로그인 사용자 아이디를 저장
				HttpSession session=request.getSession();
				session.setAttribute("signInUserId", ((User)obj).getUserid());
			...
			}else { // 테이블에 아이디/패스워드가 일치하는 사용자가 없는 경우 -> 로그인 실패
				// 로그인 페이지로(signin GET)로 redirect
